    ...
    pub fn match_bids(&mut self, order: &Order) -> (i32, Vec<Fill>) {
        ...
        for i in 0..self.asks.len() {
                ...
                && self.asks[i].user_id != order.user_id
            {
                ...
                let availableQty = self.asks[i].qty - self.asks[i].filled;
                let filledQty = cmp::min(remainingQty, availableQty);
    ...
    pub fn match_asks(&mut self, order: &Order) -> (i32, Vec<Fill>) {
        ...
        for i in 0..self.bids.len() {
            if executedQty == order.qty {
                ...
                {
                    let remaining_qty = order.qty - executedQty;
                    let available_qty = self.bids[i].qty - self.bids[i].filled;
                    let filled_qty = cmp::min(remaining_qty, available_qty);
